...
public class TOIL extends BaseTest {
    @BeforeClass
    @Parameters("language")
    public void beforeClass(String language){
        PropertyManager propertyManager = new PropertyManager();
        ExtentTest extentTest = extent.createTest("Trinity Oil");
        test.set(extentTest);
        lang = language;
    ...
    public void TOILTest(String bottles, String country, int off, int insurance, int firstUpsell, int secondUpsell, int thirdUpsell) {
        ...
        ExtentTest node = test.get().createNode("Iteration "+ title)
                .info(MarkupHelper.createOrderedList(confirmPage.getList(bottles, country, off, insurance, firstUpsell, secondUpsell, thirdUpsell, lang)));
        ...
        Assert.assertEquals(confirmPage.getGrandTotal(), "$" + confirmPage.getAmount(bottles, country, off, insurance, firstUpsell, secondUpsell, thirdUpsell).get("total"));
    }
}
