...
public class TrinityOilUpSellPage extends BasePage {
    public TrinityOilUpSellPage(WebDriver driver) {
        super(driver);
    }

    By upsellForm = By.id("upsellForm");
    By declineButton = By.id("declineButton");
    By productID = By.xpath("//input[@value='1211']");

    public void accept(){
        waitInvisibility(productID);
        submitForm(upsellForm);
        logger.info("Trinity Oil UpSell accepted");
    }

    public void decline(){
        waitInvisibility(productID);
        click(declineButton);
        logger.info("Trinity Oil UpSell declined");
    }
