...
public class CheckoutPage extends BasePage {
    ...
    public String submitCheckoutForm(String bottles, int off, int insurance){
        ...
        String email = "qa+TT" + getTimeStamp() + "@redhotmarketingllc.com";
        if(off == 1) click(continuityCheckbox);
        writeText(firstNameTextbox, propertyManager.getProperty("firstName", "TT"));
        writeText(lastNameTextbox, propertyManager.getProperty("lastName", "TT"));
        writeText(emailTextbox, email);
        writeText(address1Textbox, propertyManager.getProperty("address1", "TT"));
        writeText(postalCodeTextbox, propertyManager.getProperty("zipCode", "TT"));
        writeText(cityTextbox, propertyManager.getProperty("city", "TT"));
        writeText(phoneNumberTextbox, propertyManager.getProperty("phoneNumber", "TT"));
        writeText(cardNumberTextbox, propertyManager.getProperty("cardNumber", "TT"));
        selectOptionFromSelect(cardMonthSelect, propertyManager.getProperty("cardMonth", "TT"));
        selectOptionFromSelect(cardYearSelect, propertyManager.getProperty("cardYear", "TT"));
        writeText(cardSecurityCodeTextbox, propertyManager.getProperty("cardSecurityCode", "TT"));
